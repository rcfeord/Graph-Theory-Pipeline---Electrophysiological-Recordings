...
for g = 1:length(Grps)
    ...
    for d = 1:length(AgeDiv)
        ...
        for l = 1:length(Params.FuncConLagval)
            for e = 1:length(NetMetricsE)
                eval([VNet '.' char(NetMetricsE(e)) '=' VNe '.' char(NetMetricsE(e)) '(:,l);']);
            end
            eval(['DatTemp = ' VNet ';']);
            writetable(struct2table(DatTemp), strcat('GraphTheoryAgeGroupComparisons_NetworkMeans_',eGrp,'.xlsx'),'FileType','spreadsheet','Sheet',strcat('Age',num2str(AgeDiv(d)),'Lag',num2str(Params.FuncConLagval(l)),'ms'));
        end
    end
end
...
for g = 1:length(Grps)
    ...
    for d = 1:length(AgeDiv)
        ...
        for l = 1:length(Params.FuncConLagval)
            for e = 1:length(NetMetricsC)
                eval([VNet '.' char(NetMetricsC(e)) '=' VNe '.' char(NetMetricsC(e)) '(:,l);']);
            end
            eval(['DatTemp = ' VNet ';']);
            writetable(struct2table(DatTemp), strcat('GraphTheoryAgeGroupComparisons_ElectrodeSpecific_',eGrp,'.xlsx'),'FileType','spreadsheet','Sheet',strcat('Age',num2str(AgeDiv(d)),'Lag',num2str(Params.FuncConLagval(l)),'ms'));
        end
    end
end
...
cd(HomeDir); cd(strcat('OutputData',Params.Date));
cd('AgeGroupComparisons'); cd('LagGroupDivPlots')
...
for n = 1:length(eMet)
    ...
    if Params.figMat == 1
        saveas(F1,strcat(char(eMet(n)),' LagVsDiv.fig'));
    end
    if Params.figPng == 1
        saveas(F1,strcat(char(eMet(n)),' LagVsDiv.png'));
    end
    if Params.figEps == 1
        saveas(F1,strcat(char(eMet(n)),' LagVsDiv.eps'));
    end
    close(F1)
end
...
cd(HomeDir); cd(strcat('OutputData',Params.Date));
cd('AgeGroupComparisons'); cd('NotBoxPlotsGroups')
...
cd(HomeDir); cd(strcat('OutputData',Params.Date));
cd('AgeGroupComparisons'); cd('HalfViolinPlotsGroups')
...
cd(HomeDir); cd(strcat('OutputData',Params.Date));
cd('AgeGroupComparisons'); cd('NotBoxPlotsDiv')
...
cd(HomeDir); cd(strcat('OutputData',Params.Date));
cd('AgeGroupComparisons'); cd('HalfViolinPlotsDiv')
...
cd(HomeDir); cd(strcat('OutputData',Params.Date));
cd('AgeGroupComparisons'); cd('HalfViolinPlotsGroupsElecSpecific')

eMet = {'ND','EW','NS','Eloc','BC','PC','Z'}; 
eMetl = {'node degree','edge weight','node strength','local efficiency','betweeness centrality','participation coefficient','within-module degree z-score'}; 
...
cd(HomeDir); cd(strcat('OutputData',Params.Date));
cd('AgeGroupComparisons'); cd('HalfViolinPlotsDivElecSpecific')
...
cd(HomeDir); cd(strcat('OutputData',Params.Date))
cd('AgeGroupComparisons'); cd('NodeCartography')
...
for l = 1:length(Params.FuncConLagval)
    ...
    for g = 1:length(Grps)
        ...
        for n = 1:length(eMet)
            ...
            set(h1,'facealpha',0.3)
            hold on
            plot(xt,meanTP,'Color',c,'LineWidth',3)
            xticks(xt)
            xticklabels(xtlabtext)
            xlabel('DIV')
